...
def Loadpdb(pdb=None, hetatm= True, verbose=False):
    ...
    Prev_het=0 # to keep track of residue change in HETATM section; a new Molecule object is assigned for every residue.
    ...
    for line in pdb:
        if line[0:4]=="ATOM" and line[12:16].upper().strip() not in ["OXT"]:
            ...
            if not first_res and mol.molecule_type().lower() == 'ligand':
                if (ResNo != Prev_res or Prev_chain != Chain):
            ...
            if (ResNo != Prev_het or Prev_chain != Chain) and check_het == True: #For first HETATM check_het is always False
                mol_data[Molecule.molid] = deepcopy(mol) #copy mol object into dictionary
                first_res = True
